...
def _choose_nse_record(tradingsymbol_field: Any) -> Dict[str, Any]:
    """
    Choose NSE record from tradingsymbol field if it's a list.
    """
    if isinstance(tradingsymbol_field, list) and tradingsymbol_field:
        for rec in tradingsymbol_field:
            if str(rec.get("exchange", "")).upper() == "NSE":
                return rec
        return tradingsymbol_field[0]
    if isinstance(tradingsymbol_field, dict):
        return tradingsymbol_field
    return {"exchange": "NSE", "tradingsymbol": str(tradingsymbol_field or ""), "token": ""}

class HoldingsService:
    def __init__(self, api_client: APIClient, market: MarketDataService = None, hist: HistoricalService = None):
        self.client = api_client
        self.market = market or MarketDataService(self.client)
        self.hist = hist or HistoricalService(self.client)
